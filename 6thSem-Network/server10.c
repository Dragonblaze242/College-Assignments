...
void main()
{
	...
	char buffer[512];
	char* ack_log = "ACKNOWLEDGEMENT RECEIVED --> SEND NEXT DATA" ;
	...
	{		
		...
		if(recv(new_socket_descriptor,buffer,sizeof(buffer),0))
		{
			printf("\nMESSAGE RECEIVED FROM CLIENT = ");
			puts(buffer);
			send(new_socket_descriptor,ack_log,strlen(ack_log)+1,0);
		}
		else
		{
			send(new_socket_descriptor,"ERROR",strlen("ERROR")+1,0);
		}
